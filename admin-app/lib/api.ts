...
import {
    ...
    TrafficLightRequest,
    TrafficDataRequest,
    TrafficDataResponse, TrafficSwitchLogFilter, TrafficSwitchLog, EntityChangeLogFilter, EntityChangeLog,
} from './api-types';
...
export const logsApi = {
    getEntityChangeLogs: (filter: EntityChangeLogFilter) =>
        fetchApi<ApiResponse<EntityChangeLog[]>>('/api/v1/Logs/entity-changes', {
            method: 'POST',
            body: JSON.stringify(filter),
        }),

    getTrafficSwitchLogs: (filter: TrafficSwitchLogFilter) =>
        fetchApi<ApiResponse<TrafficSwitchLog[]>>('/api/v1/Logs/traffic-light-logs', {
            method: 'POST',
            body: JSON.stringify(filter),
        }),
};