...
export interface EntityChangeLogFilter {
    entityName?: string;
    entityId?: number;
...
export interface TrafficSwitchLogFilter {
    trafficLightId?: number;
    from?: string;
    to?: string;
}

export interface TrafficSwitchLog {
    id: number;
    trafficLightId: number;
    vehicleCount: number;
    greenLightDurationSeconds: number;
    initByName: string | null;
    timestamp: string;
}