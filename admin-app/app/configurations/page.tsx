...
export default function ConfigurationsPage() {
  ...
  const [trafficLights, setTrafficLights] = useState<Record<number, TrafficLight>>({});
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const [configurationsResponse, trafficLightsResponse] = await Promise.all([
          configurationApi.getAll(),
          trafficLightApi.getAll(),
        ]);
        
        setConfigurations(configurationsResponse.result || []);
        
        // Create a map of traffic light id to traffic light for easy lookup
        const trafficLightMap: Record<number, TrafficLight> = {};
        (trafficLightsResponse.result || []).forEach((light) => {
          trafficLightMap[light.id] = light;
        });
        setTrafficLights(trafficLightMap);
      } catch (err) {
  ...
  const columns: ColumnDef<Configuration>[] = [
    ...
    {
      accessorKey: "trafficLightId",
      header: "Traffic Light",
      cell: ({ row }) => {
        const trafficLightId = row.original.trafficLightId;
        const trafficLight = trafficLights[trafficLightId];
        return trafficLight ? trafficLight.name : trafficLightId;
      },
    },
    {
